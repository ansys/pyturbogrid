...
sys.path.append(os.path.join("C:/ANSYSDev/gitSRC/ansys-api-turbogrid", "src"))
...
def test_multi_blade_row_basic() -> list:
    machine = MBR()
    # machine.init_from_tgmachine(f"{dir_path}/mbr/5_stage_hannover/5_stage_hannover.TGMachine")
...
if __name__ == "__main__":
    ...
    brs = test_multi_blade_row_basic()
    end_time = time.time()
    ...
    for gsf, fas in all_face_areas.items():
        print(
            f"""machine gsf {gsf}: {', '.join([f"{value:.6e}" for value in fas.values()])}  std dev % {stddev_percent(fas)}"""
        )
    print(f"Element Counts")
    print(f"Blade Rows: {', '.join(brs)}")
    for gsf, ecs in all_element_counts.items():
        print(
            f"""machine gsf {gsf}: {', '.join(str(value) for value in ecs.values())}  std dev % {stddev_percent(ecs)}"""
        )
